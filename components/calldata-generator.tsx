...
interface CalldataGeneratorProps {
  onGenerate: (calldata: string) => void
}
...
export function CalldataGenerator({ onGenerate }: CalldataGeneratorProps) {
  ...
  const [tokenType, setTokenType] = useState<'ERC20' | 'ERC721' | 'ERC1155'>(
    'ERC20'
  )
  ...
  const handleGenerate = () => {
    ...
    try {
      ...
      if (tokenType === 'ERC20') {
        ...
        const decimalsNum = Number.parseInt(decimals)
        if (isNaN(decimalsNum) || decimalsNum < 0 || decimalsNum > 77) {
          setError('Invalid decimals (must be between 0 and 77)')
          return
        }

        // Parse amount with specified decimals
        const amountWei = BigInt(
          Math.floor(Number.parseFloat(amount) * 10 ** decimalsNum)
        )
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to generate calldata'
      )
    }
  }
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="space-y-4 py-4">
          ...
          {(tokenType === 'ERC20' || tokenType === 'ERC1155') && (
            <div className="space-y-2">
              <Label>
                {tokenType === 'ERC20' ? 'Amount (in tokens)' : 'Amount'}
              </Label>
              <Input
